...
const QuizSort = ({ contentDetail, courseId, onLoadData, setIsCanNextQuestion }: any) => {
  ...
  const renderItem = ({ item, drag, isActive }) => {
    ...
    return (
      <TouchableOpacity style={[styles.dragItem]} onLongPress={!isSubmitted ? drag : null}>
          ...
          style={[
            ...
            !isSubmitted
              ...
              : {
                  ...
                  backgroundColor: isCorrect ? variables.colorPrimary : '#FFEBEE',
                  borderWidth: 1,
  ...
  return (
    <GestureHandlerRootView>
      <View style={styles.container}>
        ...
        {contentObj?.thumbUrl && (
          <Image source={{ uri: contentObj?.thumbUrl }} style={styles.image} resizeMode="contain" />
        )}
        <Text style={styles.instruction}>{t('arrange_the_correct_answer')}</Text>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          {isSubmitted && points > 0 && <Image source={require('@/assets/images/Check.png')} />}
          {isSubmitted && points <= 0 && <Image source={require('@/assets/images/cancel.png')} />}
          ...
          {isSubmitted && (
            ...
            >
              {points > 0 ? t('correct_answer') : t('your_answer_was_incorrect')}
            </Text>
          )}
        </View>
        ...
        {showOptions && (
          <View>
            {correctAnswer?.map((correctAnsId, i) => {
              const correctOption = initialData.find(option => option.id === correctAnsId);
              return (
                <TouchableOpacity key={correctOption.id} style={[styles.dragItem]}>
        ...
        {!isSubmitted ? (
          <TouchableOpacity
            style={[
              styles.submitButton,
              isSorted ? styles.submitButtonEnabled : styles.submitButtonDisabled,
            ]}
