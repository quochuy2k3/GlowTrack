...
const QuizTrueFalse = ({ contentDetail, setIsCanNextQuestion, onLoadData }: any) => {
  const { userAnswers } = contentDetail;
  const [answerChecked, setAnswerChecked] = useState(userAnswers?.answers?.answer);

  const [selected, setSelected] = useState(false);

  const [isCorrectChecked, setIsCorrectChecked] = useState<boolean | null>(null);
  useEffect(() => {
    if (userAnswers?.answers) {
      setIsCanNextQuestion(true);
      setIsCorrectChecked(userAnswers?.answers?.answer === userAnswers?.correctAnswers?.answer);
    }
  }, [userAnswers, setIsCanNextQuestion]);
  ...
  const onFinish = async () => {
    try {
      ...
      if (response && response.userAnswers) {
        const isCorrect =
          response.userAnswers?.answers?.answer === response.userAnswers?.correctAnswers?.answer;
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.questionText}>{contentDetail?.contentObj?.question}</Text>
      ...
      <Text style={styles.supcriptionText}>{t('select_the_correct_answer')}</Text>
      ...
      {isCorrectChecked !== null ? (
        <View style={styles.resultContainer}>
          <View style={styles.resultTextContainer}>
            ...
            >
              {isCorrectChecked ? `${t('correct_answer')}` : `${t('your_answer_was_incorrect')}`}
            </Text>
          </View>
          ...
          {Object.entries(contentDetail?.contentObj?.content).length > 0 && (
            <View>
              ...
              <Text style={styles.feedbackText}>{contentDetail?.contentObj?.feedback}</Text>
            </View>
          )}
        </View>
      ) : null}
      ...
      <TouchableOpacity
        style={[
          ...
          styles.submitButton,
          selected === false ? styles.submitButtonInactive : styles.submitButtonActive,
        ]}
      ...
      >
        <Text
          style={[
            ...
            selected === false ? styles.submitButtonTextInactive : styles.submitButtonTextActive,
          ]}
        >
          {t('submit')}
        </Text>
