...
const QuizSingleton = ({ contentDetail, courseId, onLoadData, setIsCanNextQuestion }: any) => {
  const [loading, setLoading] = useState(true);
  const { contentObj } = contentDetail;
  const [selectedAnswer, setSelectedAnswer] = useState(contentDetail?.userAnswers?.answers?.answer);
  ...
  const getOptionTextStyle = option => {
    if (!isSubmitted) {
      return option === selectedAnswer ? styles.selectedOptionText : styles.optionText;
    }
    ...
    <SafeAreaView style={styles.container}>
      ...
      {!isSubmitted && (
        <TouchableOpacity
          style={[
            styles.submitButton,
            selectedAnswer ? styles.submitButtonActive : styles.submitButtonDisabled,
          ]}
          onPress={handleSubmit}
          disabled={!selectedAnswer}
        >
          <Text style={[styles.submitButtonText, selectedAnswer && styles.submitButtonTextActive]}>
            {t('submit')}
          </Text>
        </TouchableOpacity>
      )}
      ...
      {isAnswered && (
        <>
          ...
          >
            <Text
              style={[styles.submitButtonText, selectedAnswer && styles.submitButtonTextActive]}
            >
              {t('retry')}
            </Text>
          </TouchableOpacity>
