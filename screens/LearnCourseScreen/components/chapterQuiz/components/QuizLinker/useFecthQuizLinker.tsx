...
const useFetchQuizLinker = (
  ...
  const randomColor = () => {
    ...
    tempAvailableColors = tempAvailableColors.filter(color => color.chose !== picked.chose);
  ...
  const handleGetQuizLinkerDetail = (contentDetail, courseId, onLoadData) => {
    try {
      ...
      if (contentDetail) {
        ...
        const updatedOptions = addAttributes(contentDetail.contentObj.content.options, 'options');
  ...
  const setAnswer = answer => {
    ...
    if (currentAnswer === null) {
      if (answer.linkId === '') {
        ...
        const anotherType = answer.type === 'linkOptions' ? 'options' : 'linkOptions';
        ...
        if (optionLinkChanged) {
          ...
          answer.chose = true;
          updateOptionById(answer.id, answer.type, answer);
          updateOptionById(optionLinkChanged.id, anotherType, optionLinkChanged);
          setCurrentAnswer(answer);
        }
      }
    }
      ...
      if (answer.type !== currentAnswer.type) {
        if (answer.linkId === '') {
          ...
          if (optionChanged) {
            ...
            optionChanged.chose = false;
            optionChanged.linkId = '';
            updateOptionById(optionChanged.id, optionChanged.type, optionChanged);
            answer.trueColor = currentAnswer.trueColor;
            answer.choseColor = currentAnswer.choseColor;
  ...
  const removePickedColor = trueColor => {
    tempAvailableColors = tempAvailableColors.filter(color => color.true !== trueColor);
  };
  ...
  const updateOptionById = (id, type, updatedOption) => {
    ...
    type === 'linkOptions' ? setLinkOptions(updatedList) : setOptions(updatedList);
  };
  ...
  const setConsequenceAndColorForList = () => {
    for (let i = 0; i < contentDetail.userAnswers?.correctAnswers?.answer?.length; i++) {
      ...
      if (newOption && newLinkOption) {
        const consequence = checkConsequenceById(newLinkOption.id, newOption.id);
