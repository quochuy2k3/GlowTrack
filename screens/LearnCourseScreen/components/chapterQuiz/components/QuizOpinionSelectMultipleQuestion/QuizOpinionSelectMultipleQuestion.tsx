...
const QuizOpinionSelectMultipleQuestion = ({
  ...
  const getOptionStyle = optionId => {
    if (isSubmitted) {
      return selectedAnswers.has(optionId) ? styles.correctOption : styles.option;
    }
  ...
  const getOptionTextStyle = optionId => {
    if (isSubmitted) {
      return selectedAnswers.has(optionId) ? styles.whiteText : styles.optionText;
    }
      ...
      {isSubmitted && (
        <View style={styles.feedbackContainer}>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            />
            <Text style={(styles.feedbackText, { color: variables.colorPrimary })}>
              {t('correct_answer')}
            </Text>
          </View>
          ...
          {answerExplain !== '' && (
            <View style={{ marginVertical: variables.scale(20) }}>
              <Text style={{ marginBottom: variables.scale(20) }}>{t('feedback')}</Text>
              <WebView
                originWhitelist={['*']}
