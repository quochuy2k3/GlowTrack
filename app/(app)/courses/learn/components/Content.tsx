...
const Content = ({
  ...
  const handleGetSectionCourse = async () => {
    try {
      setLoading(true);
      const response = await service.userCourses.getSectionCourse(courseId);
      if (response?.items) {
        setCourseSection(response?.items);
      }
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleGetContentCourse = async () => {
    try {
      setIsLoading(true);
      const response = await service.userCourses.getContentCourse(courseId);
      if (response) {
        setCourseContent(response?.items);
        setListContent(response?.items);
        setMetaCourseContent(response?.meta);
      }
    } catch (e) {
      console.error(e);
    } finally {
            ...
            {content?.type === "lecture" ? (
              ...
              <View>
                {content?.contentType !== "question" ? (
                  ...
                  <Text style={styles.duration}>
                    {t("question")} :{" "}
                    {t(content?.contentTypeExplanation) ||
                      content?.contentTypeExplanation}
                  </Text>
                )}
              </View>
            )}
  ...
  const renderSections = (section: any, idx: number) => {
    const sectionContent = courseContent.filter(
      (content: any) => content.section === section.id
    );
