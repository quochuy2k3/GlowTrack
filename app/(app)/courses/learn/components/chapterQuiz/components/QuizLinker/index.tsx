...
const QuizLinker = ({
  ...
  const Option = ({ option }) => {
    ...
    const getOptionStyle = () => {
      if (consequence === false)
        return { backgroundColor: "#FFE7EB", borderColor: "#FF0E39" };
      if (consequence === true)
        return { backgroundColor: trueColor, borderColor: trueColor };
      if (chose) {
        return {
    ...
    const textColor =
      consequence === false ? "#FF0E39" : consequence ? "#FFFFFF" : "#000000";
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View
        style={[styles.quizLinkerContainer, { marginTop: variables.scale(80) }]}
      >
      ...
      {button === "retry" ? (
        <View style={styles.resultContainer}>
            ...
            <>
              <View style={styles.quizLinkerContainer}>
                {answerFeedback.option.map((option, index) => (
                  <Option
                    key={`linkOption-${option.id || index}`}
                    option={option}
                  />
                ))}
              </View>
              <View
                style={[
                  styles.quizLinkerContainer,
                  { marginTop: variables.scale(80) },
                ]}
              >
          ...
          <Text style={styles.feedbackText}>
            Object-oriented programming languages include Java, Python, C++,
        ...
        <TouchableOpacity
          style={[
            styles.submitButton,
            check.answer.length > 0 ? styles.submitButtonActive : {},
          ]}
          onPress={() => {
            handleButton();
          }}
