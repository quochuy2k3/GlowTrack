...
const QuizFill = ({
  ...
  const decodedText = decodeURIComponent(
    contentDetailQuiz.contentObj.content.text
  )
    .replace(/<p>/g, "")
    .replace(/<\/p>/g, "");
  ...
  const processAnswersWithDetails = (contentDetailQuiz: any) => {
    ...
    if (contentDetailQuiz.userAnswers) {
      ...
      const userWordsArray = decodedUserText
        .split(/\s+/)
        .filter((word) => word.trim() !== "");
      ...
      while (i < parts.length && j < userWordsArray.length) {
        if (parts[i] === userWordsArray[j] && tempGroup.trim()) {
          if (
            j + 1 < userWordsArray.length &&
            parts[i] === userWordsArray[j + 1]
          ) {
      ...
      keys.forEach((key, index) => {
        ...
        const consequence =
          value === null ? null : key.toLowerCase() === value.toLowerCase();
        inputs[key] = {
          value,
          consequence,
        };
