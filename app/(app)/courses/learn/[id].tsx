...
export default function DiscoveryScreen() {
  ...
  const { id } = useLocalSearchParams<"/(app)/courses/learn/[id]">();
  ...
  useEffect(() => {
    if (listContent && listContent.length > 0 && content) {
      setIsLastItem(
        content.content === listContent[listContent.length - 1].content
      );
    }
  }, [listContent, content]);
  useEffect(() => {
  ...
  const onLoadData = async () => {
    try {
      setLoading(true);
      const response = await service.userCourses.getContentCourse(id, 1, 10);
      if (response?.items) {
        setListContent(response.items);
      }
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const getNextContent = async () => {
    ...
    const currentIndex = listContent.findIndex(
      (item) => item.content === content.content
    );
  ...
  const getDetailContent = async () => {
    ...
    try {
      const response = await service.userCourses.getDetailQuestion(
        id,
        contentId
      );
      if (response) {
          ...
          <View style={styles.tabBar}>
            {Object.keys(tabsData).map((tabKey) => (
              <TouchableOpacity
                ...
                style={[
                  styles.tabButton,
                  tabActive === tabKey && { backgroundColor: "#f0f0f0" },
                ]}
              >
                <Text
                  style={[
                    styles.tabText,
                    tabActive === tabKey && styles.tabTextActive,
                  ]}
                >
