...
import { Text, StyleSheet, ScrollView, TouchableOpacity, FlatList } from 'react-native';
...
const AddModule = () => {
  ...
  const handleAddModule = async () => {
    ...
    try {
      ...
      const resAddModule = await service.userPlans.addModule(pickedItem.name, pickedItem.id);
      if (Object.keys(resAddModule).length === 0 && resAddModule.constructor === Object)
        router.back();
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.listItemContainerItems, { flex: 1 }]}>
        {isLoading ? (
          <View style={[styles.loadingMoreSpinner, { marginTop: variables.scale(100) }]}>
            <Spinner />
          </View>
        ) : (
          <ScrollView style={{ flex: 1 }}>
            {userSettingsCareerGoal?.skillsSuggest
              ?.filter(
                skillSuggest =>
                  !userSettingsCareerGoal?.skills?.some(skill => skill === skillSuggest.id)
              )
              .map(item => (
